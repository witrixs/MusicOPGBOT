...
module.exports = async ({ inter, queue }) => {
    ...
    // Рассчитываем новую громкость, увеличивая текущую на 5
    const vol = Math.floor(queue.node.volume + 5);

    // Проверяем, не превышает ли новая громкость максимальное значение
    if (vol > maxVol) 
        return inter.editReply({ content: await Translate(`Я не могу увеличить громкость выше максимума <${inter.member}>... попробуйте снова? <❌>`) });
